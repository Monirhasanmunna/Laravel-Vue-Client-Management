<script setup>
    ...
    watch(searchInput, debounce(()=> router.get(route('account.index'), searchInput, {
        preserveScroll: true,
        preserveState: true
    }), 500));
...
<template>
    <MainLayout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h3 class="dark:text-gray-100">Account List</h3>
            </div>
                ...
                <div class="-m-1.5 overflow-x-auto">
                    <div class="p-1.5 min-w-full inline-block align-middle">
                        <div class="border rounded-lg divide-y divide-gray-200 dark:border-gray-700 dark:divide-gray-700">
                                ...
                                <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                                    ...
                                    <tbody class="divide-y divide-gray-300 dark:divide-gray-700">
                                        <tr v-for="(account, index) in accounts.data" :key="index">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                <span v-if="account.status === 'active'" class="badge-success dark:outline-success">Actice</span>
                                                <span v-else class="badge-danger dark:outline-danger">Inactive</span>
                                            </td>

                                            <!-- <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-3">
                                               <Link :href="route('account.destroy', account.id)" method="delete" preserve-scroll preserve-state type="button" as="button" class="py-2 px-3 bg-red-600 text-white rounded-md dark:outline-danger">Delete</Link>
                                               <Link :href="route('account.edit', account.id)" class="py-2 px-3 bg-blue-600 text-white rounded-md dark:outline-primary">Edit</Link>
                                            </td> -->
                                        </tr>
                            ...
                            <div class="py-3 px-4 " v-if="accounts.data.length > 0">
                                <nav class="flex items-center space-x-2">
                                    <Link :href="link.url" :class="{'bg-gray-300 dark:bg-[#141b2b] dark:text-blue-800':link.active == true}" v-for="(link, index) in accounts.links" :key="index"
